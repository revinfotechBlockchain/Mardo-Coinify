 ...
 */
add_action( 'widgets_init', 'tolarcek_video_widget' );
/* Function that registers our widget. */
function tolarcek_video_widget() {
	register_widget( 'tolarcek_videos' );
}
class tolarcek_videos extends WP_Widget {
	function __construct() {
		/* Widget settings. */
		$widget_ops = array( 'classname' => 'videos', 'description' => esc_attr__('Displays the post image and title.','tolarcek'));
		/* Create the widget. */
		parent::__construct( 'videos-widget',esc_attr__('Tolarcek - Premium Video','tolarcek'), $widget_ops, '' );
		...
		<div class="widgett <?php if(!empty($instance['self_video'])) echo 'self'; ?> id-<?php echo esc_attr($this->id); ?>">	
			<div class="video">
				...
				<?php if(!empty($instance['display_text'])) { ?>
					<div class="wttitle"><a target="_blank" href="<?php echo esc_attr($instance['link']); ?>" rel="bookmark" title="<?php esc_attr_e('Permanent Link to','tolarcek')?> <?php echo esc_attr($instance['text']); ?>"><?php echo esc_attr($instance['text']);?></a></div>
				<?php } ?>
		</div>	
	...
	function form( $instance ) {
		...
		$defaults = array( 'title' => 'Tolarcek Video', 'video' => 'https://www.youtube.com/watch?v=35YZkJaDhh8', 'text' => 'Tolarcek Video Text', 'text-top' => '200px','self_video'=>'','auto_play' => '','display_text' => '','link' =>'','self_video' => '','image' => '');
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id( 'title' )); ?>"><?php esc_attr_e('Title:','tolarcek') ?></label>
			<input class="widefat" id="<?php echo esc_attr($this->get_field_id( 'title' )); ?>" name="<?php echo esc_attr($this->get_field_name( 'title' )); ?>" value="<?php echo esc_attr($instance['title']); ?>" />
		</p>
		
		<p>
			<label for="<?php echo esc_attr($this->get_field_id( 'video' )); ?>"><?php esc_attr_e('Video URL:','tolarcek') ?></label>
			...
			<br /><small><?php esc_attr_e('(Youtube or Vimeo or self hosted)','tolarcek') ?></small>
		</p>
		<p>
		...
		<label for="<?php echo esc_attr($this->get_field_id( 'self_video' )); ?>"><?php esc_attr_e('Self hosted video (formats : "mp4", "m4v", "webm", "ogv", "wmv", "flv")?','tolarcek') ?></label>
		</p>
        <p>
            <label for="<?php echo esc_attr($this->get_field_name( 'image' )); ?>"><?php esc_attr_e('Image cover for self hosted video:','tolarcek') ?></label>
            ...
            <input class="upload_image_button" type="button" value="<?php esc_attr_e('Upload Image','tolarcek') ?>" />
        </p>		
		<p>
		...
		<label for="<?php echo esc_attr($this->get_field_id( 'auto_play' )); ?>"><?php esc_attr_e('Auto play (only for self hosted video)?','tolarcek') ?></label>
		</p>		
		<p>
		...
		<label for="<?php echo esc_attr($this->get_field_id( 'display_text' )); ?>"><?php esc_attr_e('Display Text with link?','tolarcek') ?></label>
		</p>		
		<p>
			<label for="<?php echo esc_attr($this->get_field_id( 'link' )); ?>"><?php esc_attr_e('Link URL:','tolarcek') ?></label>
			...
			<br /><small><?php esc_attr_e('(Link for the text)','tolarcek') ?></small>
		</p>			
		<p>
			<label for="<?php echo esc_attr($this->get_field_id( 'text' )); ?>"><?php esc_attr_e('Text:','tolarcek') ?></label>
			...
			<br /><small><?php esc_attr_e('(Text over the video)','tolarcek') ?></small>
		</p>		
		<p>
			<label for="<?php echo esc_attr($this->get_field_id( 'text-top' )); ?>"><?php esc_attr_e('Text bottom margin:','tolarcek') ?></label>
			...
			<br /><small><?php esc_attr_e('(Bottom margin for text with px (200px))','tolarcek') ?></small>
		</p>		
		<?php
	}
