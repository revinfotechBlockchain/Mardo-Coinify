 ...
 */
add_action( 'widgets_init', 'tolarcek_category_select_post_widget' );
/* Function that registers our widget. */
function tolarcek_category_select_post_widget() {
	register_widget( 'tolarcek_category_select_posts' );
}
class tolarcek_category_select_posts extends WP_Widget {
	function __construct() {
		/* Widget settings. */
		$widget_ops = array( 'classname' => 'category_select_posts', 'description' => esc_html__('Displays the post image and excerpt from a selected category','tolarcek') );
		/* Create the widget. */
		parent::__construct( 'category_select_posts-widget', esc_attr__('Tolarcek - Premium Posts','tolarcek'), $widget_ops, '' );
	}
		...
		<div class="widgett <?php if ( has_post_format( 'quote' , get_the_id())) echo 'quote-widget';?> <?php if ( has_post_format( 'link' , get_the_id())) echo 'link-widget';?>">		
			...
			if ( !get_post_format() || has_post_format( 'gallery' , get_the_id())) {
				...
				if ( has_post_thumbnail() ){
					$image = wp_get_attachment_image_src(get_post_thumbnail_id(get_the_ID()),'tolarcek-widget');
					$image = $image[0];}	
					?>			
	...
	function form( $instance ) {
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id( 'title' )); ?>"><?php echo esc_attr__('Title:','tolarcek') ?></label>
			<input class="widefat" id="<?php echo esc_attr($this->get_field_id( 'title' )); ?>" name="<?php echo esc_attr($this->get_field_name( 'title' )); ?>" value="<?php echo esc_attr($instance['title']); ?>" />
		</p>
		
		<p>
			<label for="<?php echo esc_attr($this->get_field_id( 'number' )); ?>"><?php echo esc_attr__('Number of posts to show:','tolarcek') ?></label>
		...
		<label for="<?php echo esc_attr( $this->get_field_id( 'excerpt' ) ); ?>"><?php echo esc_attr__('Show excpert?','tolarcek') ?></label>
		</p>	
		<p>
			<label for="<?php echo esc_attr($this->get_field_id( 'n_excerpt' )); ?>"><?php echo esc_attr__('Number of words to show in excpert:','tolarcek') ?></label>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id( 'category' )); ?>"><?php echo esc_attr__('Category to display','tolarcek') ?></label>
			<?php $args = array(
			...
			<br /><small><?php echo esc_attr__('(select category to display)','tolarcek') ?></small>
		</p>
		<p>
		<label for="<?php echo esc_attr($this->get_field_id( 'post' )); ?>"><?php echo esc_attr__('Post to display','tolarcek') ?></label>
		<select name="<?php echo esc_attr($this->get_field_name( 'post' )); ?>" id="<?php echo esc_attr($this->get_field_id( 'post' )); ?>"> <option <?php selected( $instance['post'], 'none'); ?>value="none"><?php echo esc_attr__('Display more then one post','tolarcek') ?></option><?php global $post; $args = array( 'numberposts' => -1); $posts = get_posts($args); foreach( $posts as $post ) : setup_postdata($post); ?><option <?php selected( $instance['post'], $post->ID); ?>value="<?php echo esc_attr($post->ID); ?>"><?php the_title(); ?></option> <?php endforeach; ?> </select> 		
		</p>
		<p>
		<br /><small><?php echo esc_attr__('Note: Quote post will not be displayed.','tolarcek') ?></small>	
		<br />
		</p>	
		<?php
	}
