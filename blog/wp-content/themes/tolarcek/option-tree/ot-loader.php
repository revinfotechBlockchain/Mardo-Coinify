 ...
 */
define('ICON',get_template_directory_uri() .'/images/favicon.png');
...
if ( class_exists( 'OT_Loader' ) && defined( 'OT_PLUGIN_MODE' ) && OT_PLUGIN_MODE == true ) {
  ...
  function ot_conflict_notice() {
    ...
    echo '<div class="error"><p>' . esc_html__( 'OptionTree is installed as a plugin and also embedded in your current theme. Please deactivate the plugin to load the theme dependent version of OptionTree, and remove this warning.', 'tolarcek' ) . '</p></div>';
  ...
  class OT_Loader {
    ...
    public function load_textdomain() {
      ...
      if ( OT_PLUGIN_MODE ) {
        ...
        load_plugin_textdomain( 'tolarcek', false, OT_LANG_DIR );
        
      } else {
      
        load_theme_textdomain( 'tolarcek', OT_LANG_DIR );
    ...
    public function load_option_tree() {
      ...
      /* setup the constants */
      $this->tolarcek_constants();
      
      /* include the required admin files */
      $this->admin_includes();
      
      /* include the required files */
      $this->tolarcek_includes();
     ...
     * @since     2.0
     */
    private function tolarcek_constants() {
     ...
     * @since     2.0
     */
    private function tolarcek_includes() {
     ...
     * @since     2.0
     */
    private function tolarcek_hooks() {
    ...
    public function change_image_button( $translation, $text, $domain ) {
      ...
      if ( $pagenow == apply_filters( 'ot_theme_options_parent_slug', 'themes.php' ) && 'default' == $domain && 'Insert into post' == $text ) {
        ...
        return apply_filters( 'ot_upload_text', esc_html__( 'Send to OptionTree', 'tolarcek' ) );
