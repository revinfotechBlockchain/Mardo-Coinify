...
if ( ! class_exists( 'OT_Settings' ) ) {
  ...
  class OT_Settings {
     ...
     * @since     2.0
     */
    public function tolarcek_hooks() {
      ...
      foreach( (array) $this->options as $option ) {
        ...
        foreach( (array) $this->get_pages( $option ) as $page ) {
          ...
          if ( ! isset( $page['hidden_page'] ) ) {
            ...
            /* add styles */
            add_action( 'admin_print_styles-' . $page_hook, array( $this, 'tolarcek_styles' ) );
     ...
     * @since     2.0
     */
    public function tolarcek_styles() {
      ot_admin_styles();
    }
      ...
      foreach( (array) $this->options as $option ) {
        ...
        foreach( (array) $this->get_pages( $option ) as $page ) {
          ...
          if ( ! isset( $page['hidden_page'] ) && $screen->id == $this->page_hook[$page['id']] ) {
            ...
            tolarcek_security('<div class="wrap settings-wrap" id ="page-' . $page['id'] . '">');
			  $theme = wp_get_theme();
              tolarcek_security('<h2>' . $theme->get( 'Name' ) .' '. $page['page_title'] . '</h2>');
              
              tolarcek_security(ot_alert_message( $page ));
              
              settings_errors( 'tolarcek' );
              
              /* Header */
              tolarcek_security('<div id="option-tree-header-wrap">');
                tolarcek_security('<ul id="option-tree-header">');
                  
                  tolarcek_security('<li id="pmc-tree-logo"><a href="'.LINK.'"><img src="'.ICON.'"></a></li>');
                  tolarcek_security('<li id="option-tree-version"><span>' . $theme->get( 'Name' ) . ' '. esc_attr__('Version: ','tolarcek') . $theme->get( 'Version' ).'</span></li>');
                  
                  // Add additional theme specific links here.
                  do_action( 'ot_header_list', $page['id'] );
                
                tolarcek_security('</ul>');
                
                /* layouts form */
                if ( $page['id'] == 'ot_theme_options' && OT_SHOW_NEW_LAYOUT == true )
                  ot_theme_options_layouts_form();
              
              tolarcek_security('</div>');
            
            /* remove forms on the custom settings pages */
            if ( $show_buttons ) {
              
              tolarcek_security('<form action="options.php" method="post" id="option-tree-settings-api">');
  
                settings_fields( $option['id'] );
                
            } else {
              
              tolarcek_security('<div id="option-tree-settings-api">');
              
            }
                
                /* Sub Header */
                tolarcek_security('<div id="option-tree-sub-header">');
                  
                if ( $show_buttons )
                  tolarcek_security('<button class="option-tree-ui-button button button-primary right">' . $page['button_text'] . '</button>');
                  
                tolarcek_security('</div>');
                
                /* Navigation */
                tolarcek_security('<div class="ui-tabs">');
                  
                /* check for sections */
                if ( isset( $page['sections'] ) && count( $page['sections'] ) > 0 ) {
                
                  tolarcek_security('<ul class="ui-tabs-nav">');
                  ...
                  foreach( (array) $page['sections'] as $section ) {
                    ...
                    tolarcek_security('<li id="tab_' . $section['id'] . '">'.$font.'<a href="#section_' . $section['id'] . '">' . $section['title'] . '</a></li>');
					}
                  }
                  ...
                  /* sections */
                  tolarcek_security('<div id="poststuff" class="metabox-holder">');
                
                    tolarcek_security('<div id="post-body">');
                    
                      tolarcek_security('<div id="post-body-content">');
                  ...
                  tolarcek_security('<div class="clear"></div>');
              ...
              if ( $show_buttons ) {
                ...
                tolarcek_security('<div class="option-tree-ui-buttons">');
              ...
              tolarcek_security($show_buttons ? '</form>' : '</div>');
    ...
    public function display_setting( $args = array() ) {
      ...
      /* get the option HTML */
      tolarcek_security(ot_display_by_type( $_args ));
    }
    ...
    public function do_settings_sections( $page ) {
      ...
      foreach ( (array) $wp_settings_sections[$page] as $section ) {
        ...
        tolarcek_security('<div id="section_' . $section_id . '" class="postbox ui-tabs-panel">');
          ...
          tolarcek_security('<div class="inside">');
    ...
    public function do_settings_fields( $page, $section ) {
      ...
      foreach ( (array) $wp_settings_fields[$page][$section] as $field ) {
        ...
        tolarcek_security('<div id="setting_' . $field['id'] . '" class="'.$heading.' ' . $class . '"' . $conditions . '>');
          
          tolarcek_security('<div class="format-setting-wrap">');
          
            if ( $field['args']['type'] != 'textblock' && ! empty( $field['title'] ) ) {
            
              tolarcek_security('<div class="format-setting-label">');
          
                tolarcek_security('<h3 class="label">' . $field['title'] . '</h3>');     
